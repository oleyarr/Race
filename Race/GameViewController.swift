...
class GameViewController: UIViewController {
    ...
    var carArray = [UIImage(named: "yellow_car"), UIImage(named: "red_car"), UIImage(named: "black_car"), UIImage(named: "pickup_car")]
        ...
        UIView.animate(withDuration: 0.1, delay: 0, options: .curveLinear) {
            ...
            if self.motionManager.isGyroAvailable {
                ...
                self.motionManager.startGyroUpdates(to: .main) { (data, error) in
                    ...
                    if self.carImageView.frame.intersects(self.greenViewRight.frame) || self.carImageView.frame.intersects(self.greenViewLeft.frame)
                    {
                        self.isCrash = true
                        self.gameOver()
                    }
    ...
    func createAndMoveFirstLongLane() {
        ...
        lane.frame.origin.x = gamePlaceView.center.x - lane.frame.width / 2
        lane.backgroundColor = .white
        UIView.animate(withDuration: TimeInterval(self.gamePlaceView.frame.height / AnimateLaneStep) * carAnimationDuration, delay: 0, options: .curveLinear) {
    ...
    func generateOncomingCars() {
        let car = UIImageView()
        oncomingCarInit(car)
        if let car_image = carArray.randomElement() {
            car.image = car_image
        }        
    ...
    func oncomingCarInit(_ car: UIImageView) {
        ...
        car.frame.origin.x = CGFloat.random(in: self.gamePlaceView.frame.minX + 70...self.gamePlaceView.frame.maxX - 70 - car.frame.width)
    ...
    func gameOver() {
        ...
        userDefaults.setValue("\(dateString):    \(String(sessionScores))\n\(listOfSessionScores)", forKey: "list of session scores")
    ...
    func saveAllSettings() {
        ...
        for index in 0...gameSettingsArray.count - 1 {
            print( formatter.string(from: gameSettingsArray[index].time), gameSettingsArray[index].carColor, gameSettingsArray[index].driverName, gameSettingsArray[index].scores)
        }
    }
}
